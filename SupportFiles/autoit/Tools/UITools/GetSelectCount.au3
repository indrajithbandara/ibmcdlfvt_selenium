...
If $CmdLine[0]<1 Then 
	...
	Exit
EndIf
ClipPut(getSelectCount($CmdLine[1]));
;ClipPut(getSelectCount("Downloaded Program Files"));
Sleep(2000)

Func getSelectCount($WinTitle)

;Dim $WinTitle = "Select Document Type"
;Dim $String= "Default"
;get user path 
...
WEnd
Dim $ItemNumber
$ItemNumber = ControlListView($WinTitle, "", "SysListView321", "GetSelectedCount")

;tell RFT we are done here

;MsgBox(0, "Debug", $ItemNumber)
if ($ItemNumber=1) Then
	Return GetReturnStr($Success,"Success to get select count")
Else
	Return GetReturnStr($Failed,"Failed to get select count")
EndIf
EndFunc
