...
If $CmdLine[0]<4 Then 
	...
	Exit
EndIf
ClipPut(EditFileProps($CmdLine[1],$CmdLine[2],$CmdLine[3],$CmdLine[4]))
Sleep(2000)
...
;-------------------------------------------------------------------------------
Func EditFileProps($parentFolderTitle,$fileName,$fileTitle,$fileDesc)
	...
	Dim $title = ControlGetText($fileName,"","Edit1")
	Dim $desc=ControlGetText($fileName,"","Edit2")
	ControlClick($fileName,"","Button4")
	If (StringCompare($title,$fileTitle)=0 and StringCompare($desc,$fileDesc)=0) Then
		...
		EndIf
	Else
		Return GetReturnStr($Failed,"Strings don't match " & $title & ";" & $desc)
	EndIf	
