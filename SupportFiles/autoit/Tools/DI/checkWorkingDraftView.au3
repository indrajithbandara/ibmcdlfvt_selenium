...
ConsoleWrite(checkWorkingDraftView($CmdLine[1]))

Func checkWorkingDraftView($placeName)
	...
	$hNode = _TreeView_FindNode($hTree, "Working Drafts");
	if $hNode = 0 then 
	;WinClose($parentFolderTitle)
	ConsoleWrite("Working Views doesn't exit")
	EndIf
	
	_TreeView_Click($hTree, $hNode,"Left",True)
	Sleep(1000)
	
	WinClose("Working Drafts")
