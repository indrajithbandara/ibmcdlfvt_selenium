...
Func DeleteFolder($parentFolderTitle,$folderName)
	...
	Dim $result=WinWaitActive($LOTUSQUICKR,$DeleteText_EN,$ResponseTime) ;Lotus Quickr:Are you sure you want to delete the selected item?
	If $result Then
		ControlSend($LOTUSQUICKR,$DeleteText_EN,"Button1","{ENTER}")
	Else
		;winclose($parentFolderTitle)
		Return GetReturnStr($Failed,"Dialog for confirming delete doesn't appear")
	EndIf
	$result=WinWaitActive($LOTUSQUICKR,$HaveAccessText_EN,$ResponseTime/10) ;Lotus Quickr:Sorry, you might not have access to the content you are trying reach.
	If $result Then
		ControlSend($LOTUSQUICKR,$HaveAccessText_EN,"Button1","{ENTER}")
		;winclose($parentFolderTitle)
