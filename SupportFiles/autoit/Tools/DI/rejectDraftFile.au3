...
ConsoleWrite(rejectFile($CmdLine[1],$CmdLine[2]))


Func rejectFile($placeName,$fileName)
	...
	Dim $itemIndex=_Menu_FindItem($iPopupMenu,"Reject") 
	;MsgBox(0,'',$itemIndex)
	if $itemIndex= -1 Then
		;WinClose($parentFolderTitle)
		Send("{ESC}") ;close the pop menu
		ConsoleWrite("can't find Reject menu item in the context menu!")
	EndIf
