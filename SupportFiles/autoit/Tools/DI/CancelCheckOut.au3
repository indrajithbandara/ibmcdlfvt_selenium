...
ClipPut(CancleCheckOut($CmdLine[1]))
Sleep(2000)
MsgBox(0,"Return:",ClipGet())
;-------------------------------------------------------------------------------
;tihs method is used to cancel checkout the file in office
...
If WinWaitActive($LOTUSQUICKR,"close this document",$ResponseTime/10) Then ;"do you want to close the document?"
	ControlClick($LOTUSQUICKR,"close this document","Button2")
	Sleep(2000)
Else
	Return GetReturnStr($Failed,"Maybe the cancel check-out menuitem is invalid!")
EndIf
	
If WinWaitActive($LOTUSQUICKR,"Cancel Checkout was successful",$ResponseTime/20) Then ;"Document Cancel Checkout was successful"
	ControlClick($LOTUSQUICKR,"Cancel Checkout was successful","Button1")
	Return GetReturnStr($Success,"Successful")	
Else
	Return GetReturnStr($Failed,"Impossible reason, successful dialog doesn't appear!")
EndIf
