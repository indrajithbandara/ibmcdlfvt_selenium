...
Func RenameFile($folderParentTitle,$oldFileName,$newFileName,$fileOrFolder)
	...
	;Send("{UP 2}{Enter}") 
	Sleep(3000)
	if WinExists($LOTUSQUICKR,'cannot be renamed') Then ;The selected document is a Draft. Drafts cannot be renamed.  Please select another document.
		Dim $text=ControlGetText($LOTUSQUICKR,"cannot be renamed","Static1")
		WinClose($LOTUSQUICKR,'cannot be renamed')
		Return GetReturnStr($Failed,$text)
	EndIf
	
	Dim $result=WinWaitActive($ReNameText_EN,$Path_EN,$ResponseTime)
	if $result=0 Then
		;winclose($folderParentTitle)
	...
	ControlSetText($ReNameText_EN,$Path_EN,"Edit1",$newName)
	ControlClick($ReNameText_EN,$Path_EN,"Button1")
	Sleep(3000)
	
	If WinExists($LOTUSQUICKR) Then
		Dim $text
		If WinExists($LOTUSQUICKR,$FolderExistTitle_EN) Then
			$text=ControlGetText($LOTUSQUICKR,"","Static2")
		Else
