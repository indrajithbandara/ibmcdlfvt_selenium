...
;	Dim $handle = WinGetHandle($fromFolderTitle) ;
	...
	Sleep(5000)
	Dim $text1=""
	;might get a file already exists thing...in which case just say yes
	If WinExists($PluginDialog_EN, $FileExists_EN) Then  ;file might already exist, if so overwrite it
			ControlClick($PluginDialog_EN,"","Button2")
	EndIf
	
	Do ;sleep until Upload dialog goes away
		Sleep(1000)
	Until not WinExists($Upload_EN)
	;sleep for sec to give dialog time to come up
	Sleep(1000)
	If WinExists($PluginDialog_EN) Then  ;upload is done so click OK and set to pass
			$text1=ControlGetText($PluginDialog_EN,"","Static2")
			;MsgBox(0,"Check", $text1)
			;upload complete.check text and click OK
			ControlClick($PluginDialog_EN,"","Button1")
	EndIf	
	
	If StringInStr($text1, $UploadComplete_EN) == 0 Then
