...
Sleep(2000)
;-------------------------------------------------------------------------------
;tihs method is used to add some libraries in DI, but via context menu of "Lotus Quickr", not via context menu of right blank pane
...
Func AddPlacesInDI($serverURL,$userName,$userPassword,$rememberPWDOrNot,$libNameList)
;#comments-start
...
EndIf
Dim $itemIndex=_Menu_FindItem($iPopupMenu,"Add Places...") 
if $itemIndex= -1 Then
	;WinClose($LOTUSQUICKR)
...
EndIf
;Sleep(6000)
While not ControlCommand($addPlaceTitle,$selectLibrary,"Button7","IsEnabled","")
	Sleep(2000)
WEnd
...
For $i=1 To $libraries[0]
	$index=ControlListView($addPlaceTitle,"",$ctrl,"FindItem",$libraries[$i])
	If $index<>-1 Then
		;MsgBox(0,$index,"")
		$flag=$index
...
EndIf
If $flag=-1 Then
	Return GetReturnStr($Failed,"All specified libraries don't exist")
Else
