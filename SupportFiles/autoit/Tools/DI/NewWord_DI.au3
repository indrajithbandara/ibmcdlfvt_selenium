...
ClipPut(NewWord($CmdLine[1],$CmdLine[2]))
Sleep(2000)
...
;-------------------------------------------------------------------------------
Func NewWord($parentFolderTitle,$wordName)
	...
	Sleep(1000)
	Send("{UP 2}{RIGHT}{Down 3}{ENTER}"); new word document
	;Sleep(1000)
	if WinWaitActive($NewCreateWord_EN,"",$ResponseTime)=0 Then
		;winclose($parentFolderTitle)
		Return GetReturnStr($Failed,"Dialog for Creating word doesn't appear")
	EndIf
	ControlSetText($NewCreateWord_EN,"","Edit1",$wordName)
	;ControlSetText($NewCreateWord_EN,"","Edit2",$description)
	ControlSend($NewCreateWord_EN,"","Button1","{ENTER}");here maybe appear many cases
	...
	If WinExists($LOTUSQUICKR,$NoRightToAccessTitle_EN) Then ;No right to create the folder
		...
		;ControlClick($NewCreateWord_EN,"","Button2","{ENTER}")
		WinClose($NewCreateWord_EN)
		winclose($parentFolderTitle)
	...
	ElseIf WinExists($LOTUSQUICKR,$FolderInvalidText_EN)Then ;The folder name contains invalid char, so return false
		...
		;MsgBox(0,"Info","Create Folder Failed for invalid char",1)
		;WinWaitActive($NewCreateWord_EN)
		winclose($parentFolderTitle)
		WinClose($NewCreateWord_EN)
	...
	Else	;create folder successfully
		...
		Else
			...
			;winclose($parentFolderTitle)
			Return GetReturnStr($Failed,"Create word file:"&$wordName&".doc"&" successfully, but can't find it")
		EndIf
	EndIf
EndFunc
