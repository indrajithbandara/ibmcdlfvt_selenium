...
;-------------------------------------------------------------------------------
Func NewPPT($placename,$parentFolderTitle,$wordName)
	If WinExists($MicrosoftPowerpoint_EN) Then
		WinSetState($MicrosoftPowerpoint_EN,"",@SW_MAXIMIZE)
		WinActivate($MicrosoftPowerpoint_EN)
	EndIf
	...
	If WinWaitActive($selectDocType,$selectDocText,$ResponseTime) = 0 Then
		Return GetReturnStr($Failed,$selectDocType&" dialog for Creating Powerpoint doesn't appear")
	EndIf
