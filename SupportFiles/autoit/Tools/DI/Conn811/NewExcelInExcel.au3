...
;-------------------------------------------------------------------------------
Func NewExcel($placename,$parentFolderTitle,$wordName)
	If WinExists($MicrosoftExcel_EN) Then
		WinSetState($MicrosoftExcel_EN,"",@SW_MAXIMIZE)
		WinActivate($MicrosoftExcel_EN)
	EndIf
	...
	If FileExists("c:\Program Files\microsoft office\office12") Or FileExists("d:\Program Files\microsoft office\office12") Then
		...
		Send("!h{LEFT 6}{DOWN}{RIGHT 1}{ENTER}t")
	Else
	...
	If WinWaitActive($selectDocType,$selectDocText,$ResponseTime) = 0 Then
		Return GetReturnStr($Failed,$selectDocType&" dialog for Creating Excel doesn't appear")
	EndIf
