...
ElseIf $CmdLine[0]<3 Then 
	ClipPut(NewExcel($CmdLine[1],$CmdLine[2], "true"))
Else
	ClipPut(NewExcel($CmdLine[1],$CmdLine[2], $CmdLine[3]))
EndIf
...
;-------------------------------------------------------------------------------
Func NewExcel($parentFolderTitle,$fileName, $CheckIn)
	...
	Else	;create folder successfully
		...
		If WinWaitActive($fileName,"",$ResponseTime)=0 Then
			...
			Return GetReturnStr($Failed,"New Excel application haven't appeared!")
		EndIf
		WinClose($fileName)
		...
		Else
			...
			;winclose($parentFolderTitle)
			Return GetReturnStr($Failed,"Create excel file:"&$fileName&" successfully, but can't find it")
		EndIf
	EndIf
EndFunc
