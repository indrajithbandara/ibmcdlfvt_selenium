...
ElseIf $CmdLine[0]<3 Then 
	ClipPut(NewPPT($CmdLine[1],$CmdLine[2], "true"))
Else
	ClipPut(NewPPT($CmdLine[1],$CmdLine[2], $CmdLine[3]))
EndIf
...
;-------------------------------------------------------------------------------
Func NewPPT($parentFolderTitle,$fileName, $CheckIn)
	...
	If WinWaitActive($selectDocType,$selectDocText,$ResponseTime) = 0 Then
		Return GetReturnStr($Failed,$selectDocType&" dialog for Creating file doesn't appear")
	EndIf
	...
	if WinWaitActive($NewDocumentTitle,$NewDocumentText,$ResponseTime)=0 Then
		;winclose($parentFolderTitle)
		Return GetReturnStr($Failed,"Dialog for Creating file doesn't appear")
	EndIf
	; fill in word filename
	$fileName=$fileName&".ppt"
	ControlSetText($NewDocumentTitle,$NewDocumentText,"Edit1",$fileName)
	...
	If WinExists($LOTUSQUICKROld,"OK") Then
		Dim $text
		If WinExists($LOTUSQUICKR,$FolderExistTitle_EN) Then
			$text=ControlGetText($LOTUSQUICKROld,"OK","Static2")
		Else
			$text=ControlGetText($LOTUSQUICKROld,"OK","Static1")
		EndIf
		WinClose($LOTUSQUICKROld,"OK")
	...
	Else	;create folder successfully
		...
		If WinWaitActive($fileName,"",$ResponseTime)=0 Then
			;MsgBox(0,"Oh",$fileName,1)
			Return GetReturnStr($Failed,"New Powerpoint application haven't appeared!")
		EndIf
		WinClose($fileName)
		...
		;WinSetState($parentFolderTitle,"",@SW_MAXIMIZE)
		If ControlListView($handle,"",$listView,"FindItem",$fileName)<>-1 Then
		...
		Else
			...
			;winclose($parentFolderTitle)
			Return GetReturnStr($Failed,"Create powerpoint file:"&$fileName&" successfully, but can't find it")
		EndIf
	EndIf
EndFunc
