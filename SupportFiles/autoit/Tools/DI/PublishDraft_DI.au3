...
If $CmdLine[0]<2 Then 
	...
	Exit
EndIf
ClipPut(PublishDraft($CmdLine[1],$CmdLine[2]))
Sleep(2000)
...
;-------------------------------------------------------------------------------
Func PublishDraft($parentFolderTitle,$fileName)
	...
	EndIf
	Dim $itemIndex=_Menu_FindItem($hPopupMenu,"Publish Draft") 
	
	if $itemIndex= -1 Then
		;WinClose($parentFolderTitle)
		Send("{ESC}") ;close the pop menu
		Return GetReturnStr($Failed,"can't find 'publish draft' menu item in the context menu!")
	EndIf
	If not _Menu_GetItemEnabled($hPopupMenu,$itemIndex) then 
		;WinClose($parentFolderTitle)
		Send("{ESC}") ;close the pop menu
		Return GetReturnStr($Failed,"'publish draft' menu item in the context menu is disabled!")
	EndIf
	;_Menu_ClickItem($listView,$hPopupMenu,$itemIndex)
	_Menu_ClickPopup($itemIndex,True)
	While WinExists("Publishing")
		Sleep(2000)
	WEnd
